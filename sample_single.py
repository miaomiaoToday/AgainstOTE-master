...
if __name__ == '__main__':
    ...
    for numb in range(100):
        ...
        while len(pool.finished) < config.sample.num_samples:
            global_step += 1
            print('data_id: {}.  global step: {}/{}'.format(args.data_id, global_step, config.sample.max_steps))
            ...
            #     pass
            # else:
            #     next_idx = np.random.choice(np.arange(n_tmp), p=prob, size=min(config.sample.beam_size, n_tmp),
            #                                 replace=False)
            # try:
            #     next_idx = np.random.choice(np.arange(n_tmp), p=prob, size=min(config.sample.beam_size, n_tmp), replace=False)
