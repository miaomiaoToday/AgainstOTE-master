...
class BaseDockingTask(object):
    ...
    def __init__(self, pdb_block, ligand_rdmol, ligand_position):
        super().__init__()
        self.pdb_block = pdb_block
        self.ligand_rdmol = ligand_rdmol
        self.ligand_position = ligand_position
...
class QVinaDockingTask(BaseDockingTask):
    ...
    @classmethod
    def from_generated_data_mod(cls, data, protein_root='./data/crossdocked', pock_id=0, out_ligand_receptor_list=None, data_posin=None, **kwargs):
        # pock_id, the target id of pocket, for off-target effect.
        out_ligand_receptor = out_ligand_receptor_list[pock_id]
        # ['sample_10_2023_03_15__13_22_44', 'CD38_HUMAN_44_300_0/3dzh_A_rec_3u4i_cvr_lig_tt_docked_0.sdf',
        # 'CD38_HUMAN_44_300_0\\3dzh_A_rec_3u4i_cvr_lig_tt_docked_0_pocket10.pdb']
        exp_dir = out_ligand_receptor[0]
        ligand = out_ligand_receptor[1]
        receptor = out_ligand_receptor[2]
        protein_path = os.path.join(protein_root, receptor)
        print('Here is generating qvid task: {} and {}'.format(data.ligand_filename, receptor))
        with open(protein_path, 'r') as f:
            pdb_block = f.read()
        ligand_rdmol = reconstruct_from_generated_with_edges(data)
        ligand_position = reconstruct_from_generated_with_edges(data_posin)
        return cls(pdb_block, ligand_rdmol, ligand_position, **kwargs)
    ...
    def __init__(self, pdb_block, ligand_rdmol, ligand_position, conda_env='adt', tmp_dir='./tmp', use_uff=True, center=None):
        super().__init__(pdb_block, ligand_rdmol, ligand_position)
        ...
        """
        # ligand_position
        ligand_position = Chem.AddHs(ligand_position, addCoords=True)
        if use_uff:
            try:
                not_converge = 10
                while not_converge > 0:
                ...
                pass
        # this may be relevant to rmsd_ref
        # self.ligand_rdmol = ligand_position                # not with rmsd, just for nothing.
        # self.noH_rdmol = Chem.RemoveHs(ligand_position)    # this one for rmsd_ref
