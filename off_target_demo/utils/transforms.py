...
class RefineData(object):
    ...
    def __call__(self, data):
        ...
        if torch.sum(is_H_ligand) > 0:
            ...
            data.ligand_bond_index = torch.tensor(index_changer)[old_ligand_bond_index.to(torch.int64)]
...
class FeaturizeProteinAtom(object):
    ...
    def __call__(self, data:ProteinLigandData):
        ...
        """for solving one_hot is only applicable to index tensor."""
        amino_acid = F.one_hot(data.protein_atom_to_aa_type.to(torch.int64), num_classes=self.max_num_aa)
...
class LigandCountNeighbors(object):
    ...
    def count_neighbors(edge_index, symmetry, valence=None, num_nodes=None):
        ...
        """add for transform torch.float32 to torch.int64"""
        valence = torch.tensor(valence, dtype=torch.int64)
...
class LigandRandomMask(object):
    ...
    def __call__(self, data:ProteinLigandData):
        ...
        if data.ligand_bond_index.size(1) != 0:
            """add for subgraph torch.int64"""
            data.ligand_context_bond_index, data.ligand_context_bond_type = subgraph(
                context_idx,
                # data.ligand_bond_index,
                # edge_attr = data.ligand_bond_type,
                data.ligand_bond_index.to(torch.int64),
                edge_attr=data.ligand_bond_type.to(torch.int64),
                relabel_nodes = True,
            )
        else:
...
class LigandBFSMask(object):
    ...
    def __call__(self, data):
        ...
        # new bond with ligand context atoms
        if data.ligand_bond_index.size(1) != 0:
            """subgraph from torch.float32 to torch.int64"""
            data.ligand_context_bond_index, data.ligand_context_bond_type = subgraph(
                context_idx,
                # data.ligand_bond_index,
                # edge_attr = data.ligand_bond_type,
...
class FocalBuilder(object):
    ...
    def __call__(self, data:ProteinLigandData):
        ...
        ligand_masked_pos = data.ligand_masked_pos.to(torch.int64)
        protein_pos = data.protein_pos.to(torch.int64)
        context_idx = data.context_idx.to(torch.int64)
        masked_idx = data.masked_idx.to(torch.int64)
        old_bond_index = data.ligand_bond_index.to(torch.int64)
        # old_bond_types = data.ligand_bond_type  # type: 0, 1, 2
...
class EdgeSample(object):
    ...
    def __call__(self, data:ProteinLigandData):
        ...
        # old_bond_index = data.ligand_bond_index
        # old_bond_types = data.ligand_bond_type  # type: 0, 1, 2
        # ligand_context_pos = data.ligand_context_pos.to(torch.int64)
        # ligand_masked_pos = data.ligand_masked_pos.to(torch.int64)
        context_idx = data.context_idx
        masked_idx = data.masked_idx
        old_bond_index = data.ligand_bond_index.to(torch.int64)
        old_bond_types = data.ligand_bond_type.to(torch.int64)  # type: 0, 1, 2
