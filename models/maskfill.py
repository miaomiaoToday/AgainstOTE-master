...
class MaskFillModelVN(Module):
    ...
    def __init__(self, config, num_classes, num_bond_types, protein_atom_feature_dim, ligand_atom_feature_dim):
        ...
        """3. new layer projector"""
        # self.projector = get_encoder_vn(config.projector)
    ...
    def get_loss(self, pos_real, y_real, pos_fake,  # query real positions,
        ...
        """3 embeddings"""
        h_compose = embed_compose(compose_feature, compose_pos, idx_ligand, idx_protein,
                                      self.ligand_atom_emb, self.protein_atom_emb, self.emb_dim)        # 0: [237, 256], 1: [237, 64, 3]; 0: [446, 256], 1: [226, 63, 3]
        h_ligand = embed_compose(compose_feature, compose_pos, idx_ligand, idx_protein,
                                  self.ligand_atom_emb, self.protein_atom_emb, self.emb_dim, type='ligand')
        # h_protein = embed_compose(compose_feature, compose_pos, idx_ligand, idx_protein,
        #                          self.ligand_atom_emb, self.protein_atom_emb, self.emb_dim, type='protein')
        # # Encode compose
        """3 embeddings"""
        h_compose = self.encoder(
            node_attr = h_compose,
            ...
            edge_index = compose_knn_edge_index,
            edge_feature = compose_knn_edge_feature,
        )   # (N_p+N_l, H)                                                                              # 0: [237, 256], 1: [237, 64, 3]; 0: [446, 256], 1: [226, 63, 3], take h_compose out for support
        h_ligand = self.encoder(
            node_attr=h_ligand,
            pos=compose_pos,
            edge_index=compose_knn_edge_index,
            edge_feature=compose_knn_edge_feature,
        )  # (N_p+N_l, H)
        # h_protein = self.encoder(
        #     node_attr=h_protein,
        #     pos=compose_pos,
        #     edge_index=compose_knn_edge_index,
        #     edge_feature=compose_knn_edge_feature,
        ...
        """3. projector"""
        # h_compose = self.projector(
        #     node_attr=h_compose,
        #     pos=compose_pos,
        #     edge_index=compose_knn_edge_index,
        #     edge_feature=compose_knn_edge_feature,
        # )
        # h_ligand = self.projector(
        #     node_attr=h_ligand,
        #     pos=compose_pos,
        #     edge_index=compose_knn_edge_index,
        #     edge_feature=compose_knn_edge_feature,
        # )
        # h_protein = self.projector(
        #     node_attr=h_protein,
        #     pos=compose_pos,
        #     edge_index=compose_knn_edge_index,
        #     edge_feature=compose_knn_edge_feature,
        ...
        # h_ligand = h_norm(h_ligand)
        # # h_protein = h_norm(h_protein)

        # # h_compose[0] = self.linear(h_compose[0].transpose(-2, -1).float()).transpose(-2, -1)
        # # h_compose[1] = self.linear(h_compose[1].transpose(-2, -1).float()).transpose(-2, -1)
        # h_ligand[0] = self.linear(h_ligand[0].transpose(-2, -1).float()).transpose(-2, -1)
        # h_ligand[1] = self.linear(h_ligand[1].transpose(-2, -1).float()).transpose(-2, -1)
        # # h_protein[0] = self.linear(h_protein[0].transpose(-2, -1).float()).transpose(-2, -1)
        # # h_protein[1] = self.linear(h_protein[1].transpose(-2, -1).float()).transpose(-2, -1)

        # y_protein_frontier_pred, y_protein_frontier, y_frontier_pred, y_frontier, abs_pos_mu, pos_sigma, pos_pi, pos_generate, y_real_pred, y_real, edge_pred, edge_label, y_fake_pred
        # return loss, loss_frontier, loss_pos, loss_cls, loss_edge, loss_real, loss_fake, torch.nan_to_num(loss_surf)  # loss_notpos
        # return loss, loss_frontier, loss_pos, loss_cls, loss_edge, loss_real, loss_fake, torch.nan_to_num(loss_surf), \
        #     (y_protein_frontier_pred, y_protein_frontier, y_frontier_pred, y_frontier, abs_pos_mu, pos_sigma, pos_pi, pos_generate, y_real_pred, y_real, edge_pred, edge_label, y_fake_pred)
        return loss, loss_frontier, loss_pos, loss_cls, loss_edge, loss_real, loss_fake, torch.nan_to_num(
            loss_surf), h_compose, h_ligand, h_protein
