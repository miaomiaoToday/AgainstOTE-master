...
def embed_compose(compose_feature, compose_pos, idx_ligand, idx_protein,
                  ...
                  emb_dim, type='compose'):
    """Support embedding for compose, ligand and protein"""
    ...
    if type == 'compose':
        ...
        h_sca[idx_ligand] = h_ligand[0]
        h_vec[idx_ligand] = h_ligand[1]
    elif type == 'protein':
        h_sca[idx_protein] = h_protein[0]
        h_vec[idx_protein] = h_protein[1]
    else:
        print('not supported feature type')
...
def get_compose_knn_graph(
    ...
    compose_knn_edge_feature[idx_bond[idx_bond >= 0]] = F.one_hot(compose_bond_type[idx_bond >= 0], num_classes=4).to(
        torch.float32)  # 0 (1,2,3)-onehot
...
def get_edge_atten_input(edge_index_query, n_query, context_bond_index, context_bond_type):
    if (len(edge_index_query) != 0) and (edge_index_query.size(1) > 0):
        ...
        if context_bond_index.size(1) > 0:
            ...
            adj_mat = torch.zeros([n_context, n_context], dtype=torch.long, device=device) - torch.eye(n_context,
                                                                                                       dtype=torch.long,
                                                                                                       device=device)
